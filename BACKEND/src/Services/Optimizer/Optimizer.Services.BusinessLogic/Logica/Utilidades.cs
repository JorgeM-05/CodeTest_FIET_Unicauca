namespace Optimizer.Services.business_logic.Logica
{
    public static class Utilidades
    {
        ...
        /// <returns></returns>
        public static int DivisionSintetica(int[] renglonCA, int[] lineaDeJ, int[] v, int t, out int comodin)
        {
            comodin = 0;
            var suma = renglonCA[lineaDeJ[0]];  // 3 -1 2
            for( var k=0; k < lineaDeJ.Length;k++)
            {
                if (renglonCA[lineaDeJ[k]] == -1) comodin++;
            }
            for (var i = 1; i < t; i++)
            {
                suma = suma * v[lineaDeJ[i]] + renglonCA[lineaDeJ[i]];
                //if (renglonCA[lineaDeJ[i]] == -1) comodin++; // cuantos comodines en la fila
            }
            return suma;
        }
